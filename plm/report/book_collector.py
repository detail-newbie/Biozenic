...
class BookCollector(object):
    ...
    def getNextPageNumber(self, mediaBox, docObject):
        ...
        if isinstance(self.customText, tuple):
            ...
            msg_vals = self.evalDictVals(
                msg_vals, docObject, self.pageCount, self.poolObj.user
            )
            end_msg = msg % msg_vals
            cha = len(end_msg)
            c.drawRightString(
                float(x1) - cha, self.bottomHeight, " Page: %r" % self.pageCount
            )
            c.drawString(float(x) + 20, self.bottomHeight, end_msg)
        else:
            c.drawRightString(
                float(x1) - 50, self.bottomHeight, "Page: %r" % self.pageCount
            )
        self.pageCount += 1
            ...
            try:
                if self.jumpFirst:
                    ...
                    try:
                        ...
                        self.poolObj.get("ir.attachment").advancedPlmReportEngine(
                            document=docObject,
                            canvas=canvas,
